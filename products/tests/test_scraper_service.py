...
class ScraperServiceTestCases(TestCase):
    ...
    @patch('products.scrapers.best_buy_scraper.BestBuyScraper')
    @patch('products.scrapers.amazon_scraper.amazon_scraper')
    def test_update_products_empty(self, mock_best_buy_scraper, mock_amazon_scraper):
        ...
        # Setting up mocks
        mock_best_buy_scraper.get_price_bestbuy.return_value =\
            (False, 42.00, "2")
        mock_best_buy_scraper.get_product_url_bestbuy.return_value = 'nya2.net'
        mock_amazon_scraper.return_value = [(True, 9.50, "1"), (True, 0.05, "3")]
    ...
    def test_update_products_one_amazon(self, mock_best_buy_scraper, mock_amazon_scraper):
        ...
        # Setting up mocks
        mock_best_buy_scraper.get_price_bestbuy.return_value = (False, 42.00, "2")
        mock_best_buy_scraper.get_product_url_bestbuy.return_value = 'nya2.net'
        mock_amazon_scraper.return_value = (False, 0.05, "3")
        ...
        # Grabbing full object and testing for changes
        best_buy_object = Product.objects.get(product_name='3')
        self.assertFalse(best_buy_object.current_stock, "Update to product should have changed stock to out-of-stock")
        self.assertNotEqual(best_buy_object.last_updated, original_timestamp,
                            "Updating product should change the timestamp")
